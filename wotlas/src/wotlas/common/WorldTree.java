...
public class WorldTree extends JPanel {
    ...
    public WorldTree(WorldManager wManager) {
        this(wManager, WorldTree.ALL_LEVELS);
    }
    ...
    public WorldTree(WorldManager wManager, byte level) {
        ...
        if (wMaps != null)
            for (int w = 0; w < wMaps.length; w++) {
                ...
                if (level == WorldTree.WORLD_LEVEL_ONLY)
                    continue;
                ...
                for (int t = 0; t < tMaps.length; t++) {
                    ...
                    if (level == WorldTree.TOWN_LEVEL_ONLY)
                        continue;
                    ...
                    for (int b = 0; b < bMaps.length; b++) {
                        ...
                        if (level == WorldTree.BUILDING_LEVEL_ONLY)
                            continue;
                        ...
                        for (int i = 0; i < iMaps.length; i++) {
                            ...
                            if (level == WorldTree.INTERIORMAP_LEVEL_ONLY)
                                continue;
        ...
        this.tree.setFont(new Font("Dialog", Font.PLAIN, 10));

        this.tree.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                int selRow = WorldTree.this.tree.getRowForLocation(e.getX(), e.getY());
                TreePath selPath = WorldTree.this.tree.getPathForLocation(e.getX(), e.getY());
