...
public class AStarDoubleServer {
    /*------------------------------------------------------------------------------------*/
    ...
    */
    public void setSpriteSize(int size) {
        this.SPRITE_SIZE = size - 1;
    }

    /** To get sprite size
    */
    public int getSpriteSize() {
        return (this.SPRITE_SIZE + 1);
    }
    ...
    */
    ...
    public boolean isNotBlock(int x, int y) {
        ...
        if ((x < 0) || (x + this.SPRITE_SIZE >= this.mapWidth) || (y < 0) || (y + this.SPRITE_SIZE >= this.mapHeight))
            return false;

        return (this.map[x][y] && this.map[x][y + this.SPRITE_SIZE] && this.map[x + this.SPRITE_SIZE][y] && this.map[x + this.SPRITE_SIZE][y + this.SPRITE_SIZE] && this.map[x + this.SPRITE_SIZE / 2][y + this.SPRITE_SIZE / 2]);
    }
    ...
    public boolean isValidGoal(Point pointGoal) {
        ...
        if (isNotBlock(x, y)) {
            if (this.SHOW_DEBUG)
                System.out.println("AStarDoubleServer \t (" + x + "," + y + ") is valid point");
            return true;
        } else {
            if (this.SHOW_DEBUG) {
                System.out.println("AStarDoubleServer \t (" + x + "," + y + ") is not a valid point -> search a valid point");
                System.out.println("\ttileSize = " + this.tileSize);
                System.out.println("\tSPRITE_SIZE = " + this.SPRITE_SIZE);
                System.out.println("\tmapWidth = " + this.mapWidth);
        ...
        if (x + this.SPRITE_SIZE >= this.mapWidth) {
            //      if (SHOW_DEBUG)
            ...
            pointGoal.x = this.mapWidth - this.SPRITE_SIZE - 1;
            //        return true;
            //      }
        }
        if (y + this.SPRITE_SIZE >= this.mapHeight) {
            //      if (SHOW_DEBUG)
            ...
            pointGoal.y = this.mapHeight - this.SPRITE_SIZE - 1;
            //        return true;
            //      }
        }
        ...
        if (x + this.SPRITE_SIZE < this.mapWidth) {
            if (isNotBlock(x + this.SPRITE_SIZE, y)) {
                if (this.SHOW_DEBUG)
                    System.out.print("change x+");
                pointGoal.x += this.SPRITE_SIZE;
                return true;
            }
        }
        if ((x + this.SPRITE_SIZE < this.mapWidth) && (y + this.SPRITE_SIZE < this.mapHeight)) {
            if (isNotBlock(x + this.SPRITE_SIZE, y + this.SPRITE_SIZE)) {
                if (this.SHOW_DEBUG)
                    System.out.print("change x+ y+");
                pointGoal.x += this.SPRITE_SIZE;
                pointGoal.y += this.SPRITE_SIZE;
                return true;
            }
        }
        if (y + this.SPRITE_SIZE < this.mapHeight) {
            if (isNotBlock(x, y + this.SPRITE_SIZE)) {
                if (this.SHOW_DEBUG)
        ...
        if ((x > this.SPRITE_SIZE) && (y + this.SPRITE_SIZE < this.mapHeight)) {
            if (isNotBlock(x - this.SPRITE_SIZE, y + this.SPRITE_SIZE)) {
                if (this.SHOW_DEBUG)
                    System.out.print("change x- y+");
                pointGoal.x -= this.SPRITE_SIZE;
        ...
        if ((x > this.SPRITE_SIZE) && (y > this.SPRITE_SIZE)) {
            if (isNotBlock(x - this.SPRITE_SIZE, y - this.SPRITE_SIZE)) {
                if (this.SHOW_DEBUG)
                    System.out.print("change x- y-");
                pointGoal.x -= this.SPRITE_SIZE;
        ...
        if ((x + this.SPRITE_SIZE < this.mapWidth) && (y > this.SPRITE_SIZE)) {
            if (isNotBlock(x + this.SPRITE_SIZE, y - this.SPRITE_SIZE)) {
                if (this.SHOW_DEBUG)
                    System.out.print("change x+ y-");
                pointGoal.x += this.SPRITE_SIZE;
    ...
    * @return true if point is valid (not blocked) in the {@link #mask mask}
    */
    public boolean isValidStart(Point pointGoal) {
        return isValid(pointGoal);
    }
    ...
    private boolean isValid(Point pointGoal) {
        ...
        int x = pointGoal.x / this.tileSize;
        int y = pointGoal.y / this.tileSize;
        ...
        if (x + this.SPRITE_SIZE > this.mapWidth) {
            if (this.SHOW_DEBUG)
                System.out.println("test x near border");
            if (this.map[x - this.SPRITE_SIZE - 1][y]) {
                if (this.SHOW_DEBUG)
                    System.out.print("player near border -> change x=mapWidth-SPRITE_SIZE-1");
                pointGoal.x = this.mapWidth - this.SPRITE_SIZE - 1;
                return true;
        ...
        if (y + this.SPRITE_SIZE > this.mapHeight) {
            if (this.SHOW_DEBUG)
                System.out.println("test y near border");
            if (this.map[x][this.mapHeight - this.SPRITE_SIZE - 1]) {
                if (this.SHOW_DEBUG)
                    System.out.print("player near border -> change y=mapHeight-SPRITE_SIZE-1");
                pointGoal.y = this.mapHeight - this.SPRITE_SIZE - 1;
                return true;
        ...
        for (int step = -2; step < 3; step++) {
            if (isNotBlock(x - 2, y + step)) {
                pointGoal.x -= 2 * this.tileSize;
                pointGoal.y += step * this.tileSize;
                return true;
            }
        }
        ...
        for (int step = -1; step < 2; step++) {
            ...
            if (isNotBlock(x + step, y - 2)) {
                pointGoal.x += step * this.tileSize;
                pointGoal.y -= 2 * this.tileSize;
                return true;
            }
            //pointGoal.y=y+2;
            if (isNotBlock(x + step, y + 2)) {
    ...
    public List findPath(Point pointStart, Point pointGoal) {
        if (this.SHOW_DEBUG)
            System.out.println("AStarDoubleServer::findPath");
        ...
        NodeDouble solution = searchNode(pointStart, pointGoal);
    ...
    */
    public void setMask(boolean mask[][]) {
        this.map = mask;
        this.mapWidth = mask.length;
        this.mapHeight = mask[0].length;
    }

    /*------------------------------------------------------------------------------------*/

    public List smoothPath(List path) {
        return smoothPath1(path);
    }
    ...
    private synchronized void changeRectangle(Rectangle r, int maskTileSize, boolean value) {
        ...
        for (int i = 0; i < cWidth; i++)
            for (int j = 0; j < cHeight; j++)
                if (cx + i < this.map.length && cy + j < this.map[0].length)
                    this.map[cx + i][cy + j] = value;
    }
    ...
    */
    public void fillRectangle(Rectangle r) {
        changeRectangle(r, this.tileSize, false);
    }
    ...
    */
    public void cleanRectangle(Rectangle r) {
        changeRectangle(r, this.tileSize, true);
    }
}