...
import wotlas.common.TextEditorPanel;
...
public class ServerAdminGUI extends JFrame {
    ...
    public ServerAdminGUI() {
        ...
        this.adminTabbedPane.setPreferredSize(new Dimension(570, 400));
        mainPanel.add(this.adminTabbedPane, BorderLayout.CENTER);
        ...
        ATextArea taInfo = new ATextArea("\nWelcome to the Wotlas Server Administration Setup !\n\n\n" + "        You'll find here all the utilities needed to configure your server. " + "If it's the first time you start this setup you'll probably want to " + "edit/register your server configuration (click on the button below). " + "You can also use the Setup Wizard (button below) to update your server's " + "config.\n");
        ...
        this.adminTabbedPane.addTab("Setup", ServerDirector.getResourceManager().getImageIcon("pin.gif"), setupPanel, "Register & Setup your Server Config");

        ServerAdminGUI.transferScript = new TextEditorPanel(ServerDirector.getResourceManager(), ServerDirector.getResourceManager().getExternalTransferScript(), "This is the script we'll use for automatic config transfer :", true);

        this.adminTabbedPane.addTab("Transfer Script", ServerDirector.getResourceManager().getImageIcon("pin.gif"), ServerAdminGUI.transferScript, "Edit/View your file transfer script:");
        ...
        ATextArea taInfo2 = new ATextArea("\n\n        If you chose to never send automatically your new server's IP " + "when it changes, you'll need to use the wizard below for manual update. " + "It will perform the same operation as the transfer script.\n");
        ...
        this.adminTabbedPane.addTab("Manual Transfer", ServerDirector.getResourceManager().getImageIcon("pin.gif"), manualPanel, "Manual IP File Transfer");

        this.adminTabbedPane.addTab("Maps", ServerDirector.getResourceManager().getImageIcon("pin.gif"), new ServerMapSetup(), "Edit the server IDs of the maps");

        this.adminTabbedPane.addTab("Start-up", ServerDirector.getResourceManager().getImageIcon("pin.gif"), new TextEditorPanel(ServerDirector.getResourceManager(), ServerDirector.getResourceManager().getExternalConfigsDir() + "server.cfg", "Here is your server's start-up config (Press 'Load' to refresh) :", false), "See your server's start-up config");

        this.adminTabbedPane.addTab("Remote", ServerDirector.getResourceManager().getImageIcon("pin.gif"), new TextEditorPanel(ServerDirector.getResourceManager(), ServerDirector.getResourceManager().getExternalConfigsDir() + "remote-servers.cfg", "Here is your 'remote servers' config here (Press 'Load' to refresh) :", false), "Edit your remote-servers config");
    ...
    static public void create() {
        ...
        ServerAdminGUI.worldManager = new WorldManager(ServerDirector.getResourceManager(), true, false);
    ...
    static public WorldManager getWorldManager() {
        return ServerAdminGUI.worldManager;
    }
    ...
    static public TextEditorPanel getTransferScript() {
        return ServerAdminGUI.transferScript;
    }
