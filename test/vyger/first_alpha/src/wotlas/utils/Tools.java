...
public class Tools {
   ...
   */
     ...
     */
    public static Class[] getImplementorsOf1(String interfaceName, String packages[])
    ...
    {};
        return (Class[]) results.toArray( toReturn );
         ...
         public static Class[] getImplementorsOf(String interfaceName, String packages[])
            throws ClassNotFoundException, SecurityException {
        Class[] cls = getImplementorsOf1(interfaceName, packages);
        if (cls != null && cls.length != 0) {
            return cls;
        }
        cls = getImplementorsOf2(interfaceName, packages);
        return cls;
    }

    public static Class[] getImplementorsOf2(String interfaceName, String packages[])
            throws ClassNotFoundException, SecurityException {
        Lookup myLookup = Lookup.getDefault();
        myLookup = Lookups.metaInfServices(Tools.class.getClassLoader());
        Class ifc = Class.forName(interfaceName);
        Template ifcTemplate = new Template(ifc);
        Result lookResults = myLookup.lookup(ifcTemplate);
        Collection<Class> coll = lookResults.allClasses();
        Collection coll2 = lookResults.allItems();

        // 2 - We search the packages using the classpath
        //     Our tokenizer possesses the different classpath entries
        ArrayList<Class> results = new ArrayList<Class>(coll);
        Iterator<Class> iter = results.iterator();
        Class cl;
        boolean ok;
        while (iter.hasNext()) {
            cl = iter.next();
            ok = false;
            for (int i = 0; i < packages.length; i++) {
                if (packages[i].equalsIgnoreCase(cl.getPackage().getName())) {
                    ok = true;
                    break;
                }
