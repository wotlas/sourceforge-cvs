...
import wotlas.common.message.action.BasicActionWithPositionMessage;
import wotlas.common.message.action.BasicActionWithTargetMessage;
import wotlas.common.message.action.CastActionWithLocationMessage;
...
public class DataManager extends Thread implements NetConnectionListener, Tickable, Menu2DListener {
    ...
    */
     ...
     */
    private Hashtable<String, ScreenObject> screenObjects;
    ...
    */
     ...
     */
    public Hashtable<String, ScreenObject> getScreenObjects() {
    ...
    */
    ...
    public boolean mouseSelect(int x, int y, boolean isLeftClick) {
        ...
        if (this.myPlayer.getLocation().isTileMap()) {
            Rectangle screen = this.gDirector.getScreenRectangle();
            ...
            try {
                if (this.myPlayer.getMyTileMap().getManager().getMapMask()[tmpX][tmpY] == TileMap.TILE_NOT_FREE) {
                    getClientScreen().getMapPanel().setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
                    this.commandRequest = DataManager.COMMAND_NOTHING;
                    return true;
                }
            } catch (Exception e) {
                /*
        ...
        if (this.commandRequest != DataManager.COMMAND_NOTHING) {
            // init vars
            byte indexForMaskTarget = 0;
            byte targetRange = 0;
            WotlasLocation loc = new WotlasLocation(this.myPlayer.getLocation());
            String targetKey = "";
            ...
            */
            ...
            int range = new Double(java.lang.Math.sqrt(java.lang.Math.pow(x - this.myPlayer.getX(), 2) + java.lang.Math.pow(y - this.myPlayer.getY(), 2)) / 32).intValue();
            if (range <= 1)
            ...
            */
            ...
            if (!this.commandAction.isValidTarget(indexForMaskTarget, targetRange)) {
                if (DataManager.SHOW_DEBUG) {
            ...
            switch (indexForMaskTarget) {
                ...
                case UserAction.TARGET_TYPE_PLAYER:
                    switch (this.commandRequest) {
                        case COMMAND_CAST:
                            sendMessage(new CastActionWithTargetMessage(this.commandAction.getId(), ((ScreenObject) object).getPrimaryKey(), targetRange));
                            break;
                        case COMMAND_BASIC:
                            sendMessage(new BasicActionWithTargetMessage(this.commandAction.getId(), ((ScreenObject) object).getPrimaryKey(), targetRange));
                            break;
                    }
                    break;
                ...
                case UserAction.TARGET_TYPE_GROUND:
                    switch (this.commandRequest) {
                        case COMMAND_CAST:
                            sendMessage(new CastActionWithPositionMessage(this.commandAction.getId(), x, y, targetRange));
                            break;
                        case COMMAND_BASIC:
                            sendMessage(new BasicActionWithPositionMessage(this.commandAction.getId(), x, y, targetRange));
                            break;
                    }
                    break;
                ...
                case UserAction.TARGET_RANGE_ANY:
                    switch (this.commandRequest) {
                        case COMMAND_CAST:
                            sendMessage(new CastActionWithLocationMessage(this.commandAction.getId(), loc));
                            break;
                        case COMMAND_BASIC:
                            break;
                    }
             ...
             */
        } else if (this.commandRequest == DataManager.COMMAND_NOTHING && this.myPlayer.getLocation().isTileMap() && object instanceof PlayerImpl) {
            ...
            // Select
            this.circle = new CircleDrawable(this.selectedPlayer.getDrawable(), 20, this.selectedPlayer.getBasicChar().getColor(), true, ImageLibRef.AURA_PRIORITY);
            this.gDirector.addDrawable(this.circle);
            this.gDirector.addDrawable(this.selectedPlayer.getTextDrawable());
            if (!this.selectedPlayer.getLocation().isTileMap()) {
                this.gDirector.addDrawable(this.selectedPlayer.getBasicChar().getAura());
    ...
    */
    public synchronized boolean removeScreenObject(ScreenObject item) {
        this.screenObjects.remove(item.getPrimaryKey());
        return true;
    }
    ...
    */
    ...
    public void changeMapData() {
        ...
        try {
            ...
            } else if (this.myPlayer.getLocation().isTileMap()) {
