...
public class PropertiesConfigFile extends Properties {
    ...
    public PropertiesConfigFile(ResourceManager rManager, String configName) {
        ...
        if (rManager.inJar() && !(new File(this.configPath).exists())) {
            ...
            if (text == null || !rManager.saveText(this.configPath, text)) {
        ...
        Properties configProps = rManager.loadProperties(this.configPath);

        if (configProps == null) {
            Debug.signal(Debug.FAILURE, null, "Failed to load " + this.configPath + " !");
            Debug.exit();
        }
    ...
    protected Object setProperty(String key, String value, boolean save) {
        ...
        String oldConfig = this.rManager.loadText(this.configPath);

        if (oldConfig == null) {
            Debug.signal(Debug.ERROR, this, "Failed to load " + this.configPath + " !");
            return obj;
        }

        oldConfig = FileTools.updateProperty(key, value, oldConfig);

        if (!this.rManager.saveText(this.configPath, oldConfig))
            Debug.signal(Debug.ERROR, this, "Failed to save " + this.configPath + " !");
