...
public class TownMap extends ScreenRectangle implements WotlasMap {
    ...
    public void init(WorldMap myWorldMap) {
        ...
        for (int i = 0; i < this.mapExits.length; i++)
            this.mapExits[i].setMapExitLocation(thisLocation);
    ...
    public void initMessageRouting(MessageRouterFactory msgRouterFactory, WorldManager wManager) {
        ...
        for (int i = 0; i < this.buildings.length; i++)
            if (this.buildings[i] != null && this.buildings[i].getInteriorMaps() != null) {
    ...
    public MapExit findTownMapExit(Rectangle fromPosition) {
        ...
        if (this.mapExits == null) {
            ...
            for (int i = 0; i < bExits.length; i++) {
                if (bExits[i].getMapExitSide() == MapExit.WEST && fromPosition.x <= this.x + this.width / 2)
                    return bExits[i];

                if (bExits[i].getMapExitSide() == MapExit.EAST && fromPosition.x >= this.x + this.width / 2)
                    return bExits[i];

                if (bExits[i].getMapExitSide() == MapExit.NORTH && fromPosition.y <= this.y + this.height / 2)
                    return bExits[i];

                if (bExits[i].getMapExitSide() == MapExit.SOUTH && fromPosition.y >= this.y + this.height / 2)
                    return bExits[i];
            }
        ...
        for (int i = 0; i < this.mapExits.length; i++) {
            if (this.mapExits[i].getMapExitSide() == MapExit.WEST && fromPosition.x <= this.x + this.width / 2)
                return this.mapExits[i];

            if (this.mapExits[i].getMapExitSide() == MapExit.EAST && fromPosition.x >= this.x + this.width / 2)
                return this.mapExits[i];

            if (this.mapExits[i].getMapExitSide() == MapExit.NORTH && fromPosition.y <= this.y + this.height / 2)
                return this.mapExits[i];

            if (this.mapExits[i].getMapExitSide() == MapExit.SOUTH && fromPosition.y >= this.y + this.height / 2)
                return this.mapExits[i];
        }
    ...
    public Building isEnteringBuilding(int destX, int destY, Rectangle rCurrent) {
        ...
        for (int i = 0; i < this.buildings.length; i++) {
            ...
            if (buildRect.contains(destX, destY) && buildRect.intersects(rCurrent))
                return this.buildings[i]; // building reached
        }
