...
public abstract class UserAction implements SendObjectReady {
    ...
    static public final byte TARGET_TYPE_SELF = 0; // to use self or none
    ...
    static public final byte TARGET_TYPE_SPELL = 6;
    ...
    public boolean isValidTarget(byte targetType, byte range) {
        if (!MaskTools.isSet(this.maskTarget, targetType)) {
            // System.out.println(" type is not valid  ");
            return false;
        }
        if (this.targetRange < range) {
