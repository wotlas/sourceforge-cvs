...
public class BotManager {
    ...
    public boolean init(Properties serverProperties) {
        ...
        if (!this.botFactory.init(serverProperties))
        ...
        try {
            ...
            this.botChatService = (BotChatService) myClass.newInstance();
        } catch (Exception ex) {
            Debug.signal(Debug.ERROR, null, "Failed to create new instance of " + botChatServiceClass + ", " + ex);
            return false;
        }

        if (!this.botChatService.init(serverProperties))
            return false; // init failed

        this.botChatService.connect(); // we try a first connection
        return true;
    }
    ...
    public void shutdown() {
        ...
        this.botChatService.shutdown();

        // 2 - clean up
        this.botFactory = null;
        this.accountManager = null;
        this.botChatService = null;
    }
    ...
    public void openChatBotSession(BotPlayer bot, PlayerImpl player) {
        ...
        if (player instanceof BotPlayer) {
            Debug.signal(Debug.WARNING, this, "Bot " + player.getPrimaryKey() + " tried to talk with other bots !!");
            return;
        }

        // 2 - Check if the bot chat service is running
        if (!this.botChatService.isAvailable())
            return; // not running

        // 3 - Try to open a session
        this.botChatService.openSession(bot, player);
    }
    ...
    public void openChatBotSessions(Hashtable playerGroup, PlayerImpl player) {
        ...
        synchronized (playerGroup) {
            ...
            while (it.hasNext()) {
                ...
                this.botChatService.openSession((BotPlayer) pl, player);
            }
        }
    }
    ...
    public void closeChatBotSession(BotPlayer bot, PlayerImpl player) {
        // 1 - We don't talk to other bots... (security)
        if (player instanceof BotPlayer) {
            Debug.signal(Debug.WARNING, this, "Bot " + player.getPrimaryKey() + " tried to close a session with players.");
            return;
        }
        ...
        this.botChatService.closeSession(bot, player);
    }
    ...
    public void closeChatBotSessions(Hashtable playerGroup, PlayerImpl player) {
        ...
        synchronized (playerGroup) {
            ...
            while (it.hasNext()) {
                ...
                this.botChatService.closeSession((BotPlayer) pl, player);
            }
        }
    }
    ...
    public void refreshBotState() {
        synchronized (this.accountManager) {
            ...
            while (it.hasNext()) {
                ...
                if (pl instanceof BotPlayer)
                    pl.setIsConnectedToGame(this.botChatService.isAvailable());
            }
        }
    }
