...
public class MacroPlugIn extends JPanelPlugIn {
    ...
    public String processMacros(String text) {
        ...
        do {
            ...
            if (id > MacroPlugIn.MAX_MACROS - 1 || this.macrosFields[id] == null)
    ...
    protected void addMacro(String macro) {
        ...
        while (index < MacroPlugIn.MAX_MACROS && this.macrosFields[index] != null)
            index++;
    ...
    protected void removeMacro(int index) {
        ...
        if (this.macrosRadios[index] != null)
            this.macrosRadios[index].setSelected(true);
        else if (index - 1 >= 0 && this.macrosRadios[index - 1] != null)
            this.macrosRadios[index - 1].setSelected(true);
        else if (this.macrosRadios[0] != null)
            this.macrosRadios[0].setSelected(true);
    ...
    protected String[] getMacros() {
        ...
        while (nb < MacroPlugIn.MAX_MACROS && this.macrosFields[nb] != null)
            nb++;

        String list[] = new String[nb];

        for (int i = 0; i < nb; i++)
            list[i] = this.macrosFields[i].getText();
    ...
    protected String getMacro(int index) {
        ...
        if (index < 0 || index >= MacroPlugIn.MAX_MACROS || this.macrosFields[index] == null)
    ...
    protected int getSelectedIndex() {
        ...
        for (int i = 0; i < MacroPlugIn.MAX_MACROS && this.macrosPanel[i] != null; i++)
            if (this.macrosRadios[i].isSelected())
                return i;
    ...
    public boolean save(String[] macros) {
        ...
        if (rManager.saveObject(list, rManager.getExternalMacrosDir() + MacroPlugIn.MACROS_PREFIX + ClientDirector.getDataManager().getMyPlayer().getPrimaryKey() + MacroPlugIn.MACROS_SUFFIX))
            return true;
