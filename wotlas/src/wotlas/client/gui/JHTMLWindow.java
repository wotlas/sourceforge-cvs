...
public class JHTMLWindow extends JDialog implements ActionListener {
     ...
     */
    public JHTMLWindow(Frame frame, String title, String fileName, int width, int height, boolean center, ResourceManager rManager) {
    ...
    public JHTMLWindow(Frame frame, String header, String title, String fileName, int width, int height, boolean center, boolean modal,
        ...
        try {
            ...
            this.html = new JEditorPane("text/html", "<b>ERROR</b><br><i>" + e.getMessage() + "</i> unreachable.");
        }

        this.html.setEditable(false);
        this.html.setBorder(new EmptyBorder(0, 0, 0, 0));
        this.html.setPreferredSize(new Dimension(width - 30, height));
            ...
            public void hyperlinkUpdate(HyperlinkEvent e) {
                if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
                    if (e instanceof HTMLFrameHyperlinkEvent) {
                        ((HTMLDocument) JHTMLWindow.this.html.getDocument()).processHTMLFrameHyperlinkEvent((HTMLFrameHyperlinkEvent) e);
                    } else {
                        try {
                            JHTMLWindow.this.html.setPage(e.getURL());

                            if (JHTMLWindow.this.timer != null) {
                                JHTMLWindow.this.timer.stop();
                                JHTMLWindow.this.nbRepaints = 0;
                                JHTMLWindow.this.timer.start();
                            }
                        } catch (IOException ioe) {
                            JHTMLWindow.this.html.setText("<b>ERROR</b><br>Failed to open URL: <i>" + e.getURL() + "</i><p>An exception occured: <i>" + ioe.getMessage() + "</i>");
                        }
                    }
                }
            }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (this.nbRepaints >= JHTMLWindow.MAX_REPAINTS) {
            this.timer.stop();
            this.nbRepaints = 0;
        }
    }
