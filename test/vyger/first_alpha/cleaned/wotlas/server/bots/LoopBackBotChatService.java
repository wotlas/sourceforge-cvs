...
public class LoopBackBotChatService implements BotChatService {
    ...
    public boolean openSession(BotPlayer bot, Player player) {
        Debug.signal(Debug.NOTICE, null, "OPEN SESSION CALLED on " + player.getPrimaryKey() + " and bot " + ((PlayerImpl) bot).getPrimaryKey());
        bot.sendChatAnswer("Welcome to you " + ((PlayerImpl) player).getFullPlayerName((PlayerImpl) bot));
        return true;
    }
    ...
    public boolean closeSession(BotPlayer bot, Player player) {
        Debug.signal(Debug.NOTICE, null, "CLOSE SESSION CALLED on " + player.getPrimaryKey() + " and bot " + ((PlayerImpl) bot).getPrimaryKey());
        return true;
    }
