 ...
 *
 * IMPORTANT : this implementation is WAS NOT synchronized... 
 *             OLD NOTE : please avoid the situation
 *                        when one thread is invoking the tick() 
 *                        method and the other a setXXX()...
 *             NOW IT'S SHOULD Syncr
 *
 * @author Petrus, Aldiss, Diego
 */
public class ServerPathFollower implements MovementComposer, BackupReady {
    ...
    */
    public void setMovementMask(boolean mask[][], int maskTileSize, int playerSize) {
        Debug.signal(Debug.FAILURE, null, "setMovementMask called on ServerPathFollower!");
        /*
                aStarDoubleServer.setMask( mask );
                aStarDoubleServer.setTileSize( maskTileSize );
                aStarDoubleServer.setSpriteSize( playerSize );
         */
    }
    ...
    * @return path
    */
    public List findPath(Point a, Point b, boolean pathInRoom) {
        int tileSize = this.aStarDoubleServer.getTileSize();
        List path = this.aStarDoubleServer.findPath(new Point(a.x / tileSize, a.y / tileSize), new Point(b.x / tileSize, b.y / tileSize));
        path = this.aStarDoubleServer.smoothPath(path);
    ...
    */
    public ServerPathFollower(float xPosition, float yPosition, double orientationAngle, AStarDoubleServer aStarDoubleServer) {
        this();
        this.xPosition = xPosition;
        this.yPosition = yPosition;
        this.orientationAngle = (float) (Math.PI / 2);
        //        this.orientationAngle = orientationAngle;
        this.aStarDoubleServer = aStarDoubleServer;
    }
    ...
    */
    public void init(Player player) {
        this.screenObject = player.getScreenObject();
        this.orientationAngle = (float) (Math.PI / 2);
        if (player.getLocation().isTileMap())
            this.aStarDoubleServer = player.getMyTileMap().getAStar();
        else
            this.aStarDoubleServer = null;
        //        Debug.signal( Debug.ERROR, null, "This should never be called : init with Player on ServerPathFollower!" );
    }
    ...
    */
    public void stopMovement() {
        resetMovement();
        if (this.screenObject instanceof SpellOnTheScreen)
            ((SpellOnTheScreen) this.screenObject).getRouter().sendMessage(new ScreenObjectPathUpdateMovementMessage(this, this.screenObject.getPrimaryKey(), this.screenObject.getSyncID()));
        if (this.screenObject instanceof NpcOnTheScreen)
            ((NpcOnTheScreen) this.screenObject).getRouter().sendMessage(new ScreenObjectPathUpdateMovementMessage(this, this.screenObject.getPrimaryKey(), this.screenObject.getSyncID()));
    }
    ...
    */
    public MovementUpdateMessage getUpdate() {
        //  if(aStarDoubleServer.isInitialized())
        if (this.screenObject.getLocation().isTileMap())
            return new ScreenObjectPathUpdateMovementMessage(this, null, this.screenObject.getSyncID());
        else
            return new ScreenObjectPathUpdateMovementMessage(this, this.screenObject.getPrimaryKey(), this.screenObject.getSyncID());
    }
        ...
        */
        ...
        if (!this.screenObject.getLocation().isTileMap()) {
            ...
            if (this.walkingAlongPath) {
                ...
                if (msg.isMoving) {
                    if (ServerPathFollower.distance(msg.srcPoint, getPosition()) > ServerPathFollower.MAX_DISTANCE_DELAY || findPath(getPosition(), msg.dstPoint, this.screenObject.getLocation().isRoom()) == null)
                        takeUpdate = true;
                    else
                        recreateTrajectory(msg.dstPoint, 0);
                } else {
                    // no movement, we received the master replica's ending trajectory position
                    if (target.x != msg.srcPoint.x || target.y != msg.srcPoint.y || ServerPathFollower.distance(target, getPosition()) > ServerPathFollower.MAX_DISTANCE_DELAY) {
                        takeUpdate = true;
                    } else {
    ...
    */
    private void updateMovementAspect() {
        ...
        this.speed = this.screenObject.getSpeed();
    }
    ...
    */
    private synchronized void updatePathMovement() {
        ...
        if (deltaT >= 0.8f) {
            System.out.println("juda > 0.8");
            return; // SECURITY if a slow is encountered
        }
        if (deltaT < 0) {
            System.out.println("juda < 0");
            return; // Date has been advanced
        }
        System.out.println("juda come here too!!");
        ...
        if (this.turningAlongPath) {
            ...
            if (deltaA <= 0) {
                if (this.useEndingOrientationValue && (this.path == null || this.pathIndex >= this.path.size())) {
                    ...
                    System.out.println("juda                    OUT 1");
                    return;
                }
                this.turningAlongPath = false;
                this.orientationAngle = ServerPathFollower.angle(getPosition(), this.nextPoint);
            } else if (deltaA > Math.PI / 8) {
                System.out.println("juda                    OUT 2");
                return; // no footsteps, the angle is to great, we just turn...
            }
        }
        // 3 - Position Update
        if (this.path == null) {
            System.out.println("juda                    OUT 3");
        ...
        float deltaD = ServerPathFollower.distance(getPosition(), this.prevPoint) - ServerPathFollower.distance(this.nextPoint, this.prevPoint);
        if (deltaD >= 0) {
            this.pathIndex++;
            ...
            if (this.pathIndex >= this.path.size()) {
                ...
                if (this.useEndingOrientationValue && this.realisticRotations) {
                    ...
                    this.turningAlongPath = true;
                    System.out.println("juda                    OUT 4");
                    return;
                }
                this.useEndingOrientationValue = false;
                stopMovement();
                System.out.println("juda                    OUT 5");
                return;
            }
    ...
    */
    public void moveTo(Point endPosition) {
        ...
        this.path = findPath(startPt, new Point(endPosition.x, endPosition.y), this.screenObject.getLocation().isRoom());
        if (this.path == null) {
            if (this.walkingAlongPath)
    ...
    */
    public void recreateTrajectory(Point pDst, int movementDeltaTime) {
        this.path = findPath(new Point((int) this.xPosition, (int) this.yPosition), new Point(pDst.x, pDst.y), this.screenObject.getLocation().isRoom());
        if (this.path == null) {
            Debug.signal(Debug.ERROR, this, "Failed to re-create path !");
            if (this.screenObject instanceof SpellOnTheScreen || this.screenObject instanceof NpcOnTheScreen)
                stopMovement();
            else
                resetMovement();
            return;
    ...
    */
    synchronized public void initMovement(List path, int deltaTime) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            d += ServerPathFollower.distance(a0, a1);
            // have we found the last point crossed by our entity ?
            if (d >= totalDistance) {
                // Path approved !
                this.path = path;
                this.pathIndex = i;
                this.orientationAngle = ServerPathFollower.angle(a0, a1);
        ...
        this.xPosition = a1.x;
        this.yPosition = a1.y;
        System.out.println(this.screenObject.getName() + " here it moves .....");
        if (this.screenObject instanceof SpellOnTheScreen || this.screenObject instanceof NpcOnTheScreen)
            stopMovement();
        else
            resetMovement();
    ...
    */
    private void updateAngularNode() {
        this.nextAngle = ServerPathFollower.angle(this.prevPoint, this.nextPoint);
        this.angularDirection = 1;
        while (this.nextAngle - this.orientationAngle > Math.PI)
            this.nextAngle = (float) (this.nextAngle - 2 * Math.PI);
        while (this.nextAngle - this.orientationAngle < -Math.PI)
