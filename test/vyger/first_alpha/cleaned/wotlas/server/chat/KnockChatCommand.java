...
/** "/knock" chat command. Produces a knock sound, useful when entering a room.
 *
 * @author Aldiss
 */

public class KnockChatCommand implements ChatCommand {
    /*------------------------------------------------------------------------------------*/
    ...
    public String getCommandDocumentation() {
        return "<font size='4'> Command 'knock'</font>" + "<br><b> Syntax :</b> /knock " + "<br><b> Voice  :</b> normal voice level " + "<br><b> Descr  :</b> to knock a door." + "<br><b> Example:</b> none.";
    }
    ...
    public boolean exec(String message, PlayerImpl player, SendTextMessage response) {
        ...
        if (!player.askGrantAccessToSendFocusSound()) {
            response.setMessage("/cmd:/knock command error : <font color='red'>you must wait 10s between two knocks...</font>");
            player.sendMessage(response);
            return true;
        }

        // TO DO : check the presence of a door in the room !!!!
