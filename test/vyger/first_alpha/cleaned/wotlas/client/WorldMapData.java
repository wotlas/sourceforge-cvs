...
public class WorldMapData implements MapData {
    ...
    public void initDisplay(PlayerImpl myPlayer, DataManager dataManager) {
        ...
        if (tileMaps != null) {
            ...
            for (int i = 0; i < tileMaps.length; i++) {
                ...
                tileMapImage = new MotionlessSprite(position.x, position.y, tileMapImageID, // image
                ImageLibRef.MAP_PRIORITY // priority
                );
    ...
    public void locationUpdate(PlayerImpl myPlayer) {
        ...
        if ((this.currentWorldMapID != myPlayer.getLocation().getWorldMapID()) || (myPlayer.getLocation().getTownMapID() > -1) || (myPlayer.getLocation().getTileMapID() > -1)) {
        ...
        TileMap tileMap = worldMap.isEnteringTileMap(destination.x, destination.y, myPlayer.getCurrentRectangle());
        ...
        if (tileMap != null) {
            ...
            if (this.isNotMovingToAnotherMap) {
                ...
                myPlayer.sendMessage(new CanLeaveWorldMapMessage(myPlayer.getPrimaryKey(), tileMap.getLocation(), 20, 20, 1));
            }
