...
public class ServerChatCommand implements ChatCommand {
    ...
    public String getCommandDocumentation() {
        return "<font size='4'>Command 'server'</font>" + "<br><b> Syntax :</b> /server " + "<br><b> Voice  :</b> normal voice level " + "<br><b> Descr  :</b> to get some information on the current server." + "<br><b> Example:</b> none";
    }
    ...
    public boolean exec(String message, PlayerImpl player, SendTextMessage response) {
        ...
        if (message.equals("/server bell")) {
            ...
            response.setMessage("/cmd:You are ringing server <b> " + ourConfig.getServerSymbolicName() + " </b> (" + ourConfig.getServerID() + ") ...");
        } else {
            response.setMessage("/cmd:You are on server " + ourConfig.getServerSymbolicName() + " (" + ourConfig.getServerID() + ") located in " + ourConfig.getLocation() + "<br> <b>Admin e-mail:</b>" + ourConfig.getAdminEmail());
        }
