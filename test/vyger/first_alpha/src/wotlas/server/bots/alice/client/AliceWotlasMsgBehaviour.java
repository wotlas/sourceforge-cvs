   ...
   */
     public void doBehaviour( Object sessionContext ) {
        ...
        // We send the answer back to the bot
           GameAccount account = aManager.getAccount(botPrimaryKey);

              if(account==null) {
                 Debug.signal(Debug.ERROR,this,"Bot not found ! ("+botPrimaryKey+")"); 
                 return;
              }

              if( !(account.getPlayer() instanceof BotPlayer) ) {
                  Debug.signal(Debug.ERROR,this,"Player is not a bot ! ("+botPrimaryKey+")");
                  return;
              }

          ( (BotPlayer) account.getPlayer() ).sendChatAnswer(message);
     }
