...
//        ,picture,getMessageRouter()) );
    ...
    public boolean isBlockedByTimeStop(String primaryKey) {
        if( timeStopDur > System.currentTimeMillis() )
            return !timeStopOwner.equals(primaryKey);
        return false;
    }
    
    public boolean setTimeStop(String primaryKey) {
        if(!isTimeStopPossible)
            return false;
        if( timeStopDur > System.currentTimeMillis() )
            return false;
        timeStopOwner = primaryKey;
        timeStopDur = System.currentTimeMillis()+(1000*4);
        return true;
    }
}