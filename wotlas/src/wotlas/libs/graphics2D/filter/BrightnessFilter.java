...
public class BrightnessFilter implements DynamicImageFilter {
    ...
    public BufferedImage filterImage(BufferedImage srcIm) {
        ...
        if (BrightnessFilter.brightnessMask == null || this.brightness == 0)
            return srcIm;
    ...
    public void setBrightness(float x, float y) {
        if (BrightnessFilter.brightnessMask == null)
            return;
        ...
        if (yb < 0)
            yb = 0;
        if (xb >= BrightnessFilter.brightnessMask.length)
            xb = BrightnessFilter.brightnessMask.length - 1;
        if (yb >= BrightnessFilter.brightnessMask[0].length)
            yb = BrightnessFilter.brightnessMask[0].length - 1;

        if (BrightnessFilter.brightnessMask != null)
            this.brightness = ((float) BrightnessFilter.brightnessMask[xb][yb]) / 255;
    }
