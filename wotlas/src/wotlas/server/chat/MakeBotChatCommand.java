...
/** "/makebot" chat command. To transform your player into a bot.
 *
 * @author Aldiss
 */

public class MakeBotChatCommand implements ChatCommand {
    ...
    public String getCommandDocumentation() {
        return "<font size='4'>Command 'makebot'</font>" + "<br><b> Syntax :</b> /makebot [password] " + "<br><b> Voice  :</b> shouting voice level " + "<br><b> Descr  :</b> transforms your player into a bot." + "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;You have to enter the bot control key." + "<br><b> Example:</b> /makebot 743743OAEBX848A3439A";
    }
    ...
    public boolean exec(String message, PlayerImpl player, SendTextMessage response) {
        ...
        // 1 - We extract the password
        if (message.indexOf(' ') != 8)
            return true; // no parameters

        message = message.substring(9);

        // 2 - Is it the right password ?
        if (message.length() == 0) {
            response.setMessage("/cmd:/makebot command error:<font color='red'> no password entered</font>");
            player.sendMessage(response);
            return true;
        } else if (!message.equals(ServerDirector.getServerProperties().getProperty("bots.controlKey"))) {
            response.setMessage("/cmd:/makebot command error:<font color='red'> bad password entered ! :no: </font>");
            player.sendMessage(response);
            return true;
        }

        // 3 - Try the transformation
        boolean result = ServerDirector.getDataManager().getBotManager().getBotFactory().transformIntoBot(player, ServerDirector.getDataManager().getAccountManager());

        if (!result) {
            // failed
            response.setMessage("/cmd:/makebot command error:<font color='red'> transformation failed. </font>");
            player.sendMessage(response);
        }
