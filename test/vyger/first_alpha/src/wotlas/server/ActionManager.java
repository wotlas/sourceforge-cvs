...
public class ActionManager implements Runnable {
    ...
    */
    ...
    public ActionManager() {
        tileMaps = ServerDirector.getDataManager().getWorldManager().getWorldMaps()[0].getTileMaps();
    }
    ...
    */
    ...
    public void action(){
        ...
        for(int i=0;i<tileMaps.length;i++) {
            screenObjects = tileMaps[i].getMessageRouter().getScreenObjects();
                ...
                if( obj instanceof PlayerOnTheScreen ) {
                    ...
                    */
                    ...
                    if( zrachu.getMovementComposer().isMoving() )
                        ...
                        +debugThis( zrachu.getMovementComposer().getXPosition() )
                        +","
                        +debugThis( zrachu.getMovementComposer().getYPosition() )
                        );
                }
    ...
    public boolean checkForMovement(PlayerImpl player) {
        ...
        if( zone.isBlockedByTimeStop( key ) 
        && !charData.isFlagSet(CharData.FLAG_TIME_ANCHOR) ) {
            return false;        
        }
