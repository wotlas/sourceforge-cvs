...
public class WorldManager {
    ...
    public void loadUniverse(boolean loadDefault, boolean pleaseLoadTreeForEditor) {
        ...
        Debug.signal(Debug.NOTICE, null, "Loading Universe Data from : " + universeHome + " preloader status [" + WorldManager.PRELOADER_STATUS + "]");
        this.worldMaps = null;
        ...
        for (int w = 0; w < worldList.length; w++) {
            ...
            String townList[] = this.rManager.listUniverseDirectories(worldList[w], WorldManager.TOWN_DIR_EXT);
            ...
            PreloaderEnabled.SetClassPreloader(WorldManager.PRELOADER_STATUS);
            String tileMapList[] = this.rManager.listUniverseDirectories(worldList[w], WorldManager.TILEMAP_DIR_EXT);
            for (int t = 0; t < tileMapList.length; t++) {
                // we load the tileMap objects
                TileMap tileMap = (TileMap) this.rManager.RestoreObject(tileMapList[t] + WorldManager.TILEMAP_FILE);
                tileMap.SetPreloader(tileMapList[t] + WorldManager.TILEMAP_FILE);
                if (tileMap == null) {
            ...
            String areaMapList[] = this.rManager.listUniverseDirectories(worldList[w], WorldManager.AREA_EXT);
            for (int index = 0; index < areaMapList.length; index++) {
                String insideAreaTileMapList[] = this.rManager.listUniverseFiles(areaMapList[index], WorldManager.TILEMAP_EXT);
                for (int index2 = 0; index2 < insideAreaTileMapList.length; index2++) {
                    TileMap tileMap = (TileMap) this.rManager.RestoreObject(insideAreaTileMapList[index2]);
    ...
    public boolean saveUniverse(boolean isDefault) {
        ...
        if (isDefault) {
            ...
            for (int w = 0; w < this.worldMaps.length; w++) {
                ...
                TileMap tileMaps[] = this.worldMaps[w].getTileMaps();
                if (tileMaps != null) {
                    for (int t = 0; t < tileMaps.length; t++) {
                        ...
                        if (tileMaps[t].getAreaName().length() <= 0) {
                            String tileMapHome = worldHome + tileMaps[t].getShortName() + WorldManager.TILEMAP_DIR_EXT + "/";
                            new File(tileMapHome).mkdir();
                            if (!this.rManager.BackupObject(tileMaps[t], tileMapHome + WorldManager.TILEMAP_FILE)) {
                            ...
                            if (!this.rManager.BackupObject(tileMaps[t], areaOfTile + tileMaps[t].getShortName() + WorldManager.TILEMAP_EXT)) {
                ...
                for (int t = 0; t < towns.length; t++) {
                    ...
                    String townHome = worldHome + towns[t].getShortName() + WorldManager.TOWN_DIR_EXT + "/";
