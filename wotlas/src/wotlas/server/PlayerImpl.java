...
public class PlayerImpl implements Player, NetConnectionListener {
    ...
    /** Movement Composer
     */
    protected MovementComposer movementComposer = new PathFollower();
    ...
    public void setDefaultPlayerLocation() {
        ...
        this.location = new WotlasLocation(worldID);
    ...
    public WotlasLocation getLocation() {
        return new WotlasLocation(this.location);
    }
    ...
    public void setLocation(WotlasLocation myLocation) {
        ...
        if (this.location.isRoom() && ServerDirector.getDataManager() != null)
            this.myRoom = ServerDirector.getDataManager().getWorldManager().getRoom(this.location);
        else {
            if (this.location.isRoom())
                Debug.signal(Debug.CRITICAL, this, "Room not found !!! location is:" + this.location);
            this.myRoom = null;
        }
    ...
    public String getFullPlayerName(PlayerImpl otherPlayer) {
        if (otherPlayer == null)
            return this.lieManager.getCurrentFakeName();
        if (otherPlayer.getPrimaryKey().equals(this.primaryKey)) {
            return this.lieManager.getCurrentFakeName();
        } else {
    ...
    public void setFullPlayerName(String fullPlayerName) {
        this.lieManager.setFullPlayerName(fullPlayerName);
    }
    ...
    public MessageRouter getMessageRouter() {
        ...
        if (this.location.isWorld()) {
            WorldMap world = ServerDirector.getDataManager().getWorldManager().getWorldMap(this.location);
            if (world != null)
                return world.getMessageRouter();
        } else if (this.location.isTown()) {
            TownMap town = ServerDirector.getDataManager().getWorldManager().getTownMap(this.location);
            if (town != null)
                return town.getMessageRouter();
        } else if (this.location.isRoom()) {
            Room room = ServerDirector.getDataManager().getWorldManager().getRoom(this.location);
            if (room != null)
                return room.getMessageRouter();
        }

        Debug.signal(Debug.ERROR, this, "MessageRouter not found ! bad location :" + this.location + " player: " + this.primaryKey);
    ...
    public void connectionCreated(NetConnection connection) {
        ...
        if (this.location.isRoom()) {
            if (this.myRoom == null) {
                Debug.signal(Debug.ERROR, this, "Player " + this.primaryKey + " has an incoherent location state");
                return;
            }
            ...
            if (this.myRoom.getMessageRouter().getPlayer(this.primaryKey) != null) {
                ...
                PlayerConnectedToGameMessage pMsg = new PlayerConnectedToGameMessage(this.primaryKey, true);
    ...
    public void connectionClosed(NetConnection connection) {
        ...
        if (this.location.isRoom()) {
            ...
            msg[1] = new PlayerConnectedToGameMessage(this.primaryKey, false);
        ...
        synchronized (this.connectionLock) {
            if (this.connection != null) {
                if (ServerDirector.SHOW_DEBUG)
                    System.out.println("Player " + this.primaryKey + " sending msg: " + message);
    ...
    public void sendChatMessage(SendTextMessage message, PlayerImpl otherPlayer) {
        synchronized (this.connectionLock) {
            if (this.connection != null) {
                ...
                if (ServerDirector.SHOW_DEBUG)
                    System.out.println("Player " + this.primaryKey + " sending to:" + otherPlayer.getPrimaryKey() + " msg: " + message);

                this.connection.queueMessage(message);

                if (!this.primaryKey.equals(otherPlayer.getPrimaryKey()))
