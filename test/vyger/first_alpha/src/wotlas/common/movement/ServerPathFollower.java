 ...
 */
 ...
 /*------------------------------------------------------------------------------------*/
    ...
    */
    public List findPath(  Point a, Point b, boolean pathInRoom ) {
        int tileSize = aStarDoubleServer.getTileSize();
        List path = aStarDoubleServer.findPath( new Point( a.x/tileSize, a.y/tileSize )
        ,new Point( b.x/tileSize, b.y/tileSize ) );
        path = aStarDoubleServer.smoothPath(path);
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    public ServerPathFollower( float xPosition, float yPosition, double orientationAngle
    ,AStarDoubleServer aStarDoubleServer) {
        this();
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    public void resetMovement() {
        ...
        if( screenObject.getLocation().isTileMap() )
            aStarDoubleServer = ServerDirector.getDataManager().getWorldManager(
            ).getTileMap( screenObject.getLocation() ).getAStar();
    }
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    public void stopMovement() {
        resetMovement();
        if( screenObject instanceof SpellOnTheScreen )
            ((SpellOnTheScreen)screenObject).getRouter(
            ).sendMessage( new ScreenObjectPathUpdateMovementMessage( this, screenObject.getPrimaryKey(), screenObject.getSyncID() ) );
        if( screenObject instanceof NpcOnTheScreen )
            ((NpcOnTheScreen)screenObject).getRouter(
            ).sendMessage( new ScreenObjectPathUpdateMovementMessage( this, screenObject.getPrimaryKey(), screenObject.getSyncID() ) );
    }
        ...
        // if (!aStarDoubleServer.isInitialized()) {
            ...
            if( walkingAlongPath ) {
                ...
                if( msg.isMoving ) {
                    ...
                    || findPath( getPosition(), msg.dstPoint, screenObject.getLocation().isRoom() )==null )
                        takeUpdate = true;
                    else
  ...
  // MOVEMENT CONTROL
    ...
    */
    public void moveTo( Point endPosition) {
        ...
        path = findPath( startPt, new Point( endPosition.x, endPosition.y ),screenObject.getLocation().isRoom() );
        if( path==null ) {
            if( walkingAlongPath )
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    public void recreateTrajectory( Point pDst, int movementDeltaTime ) {
        ...
        ,new Point( pDst.x, pDst.y ), screenObject.getLocation().isRoom() );
        if( path==null ) {
            Debug.signal( Debug.ERROR, this, "Failed to re-create path !" );
            if( screenObject instanceof SpellOnTheScreen
            || screenObject instanceof NpcOnTheScreen )
                stopMovement();
            else
                resetMovement();
            return;
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
    ...
    */
    synchronized public void initMovement( List path, int deltaTime ) {
        ...
        System.out.println( screenObject.getName()+" here it moves .....");
        if( screenObject instanceof SpellOnTheScreen
        || screenObject instanceof NpcOnTheScreen )
            stopMovement();
        else
