...
public class ServerConfigManager {
    ...
    public ServerConfigManager(ResourceManager rManager) {
        ...
        this.serversRemoteHomeUnreachable = false;
        this.lastServerTableUpdateTime = 0; // none
        this.localServerID = -1; // no config to spare from updates...
    ...
    protected String getRemoteServerTable() {
        ...
        if (this.serversRemoteHomeUnreachable)
            return null;

        // Update Period Reached ?
        long now = System.currentTimeMillis();

        if (this.remoteServerTable == null || (this.lastServerTableUpdateTime + this.UPDATE_TABLE_PERIOD) <= now) {
            // yes ! we erase the old server table.
            Debug.signal(Debug.NOTICE, null, "Loading server table...");
            this.remoteServerTable = null;
            this.lastServerTableUpdateTime = now;
        }
        ...
        this.remoteServerTable = FileTools.getTextFileFromURL(this.remoteServerConfigHomeURL + "server-table.cfg");

        if (this.remoteServerTable == null || this.remoteServerTable.length() == 0) {
            Debug.signal(Debug.CRITICAL, this, "Try to load Server Table from network failed...");
            this.serversRemoteHomeUnreachable = true;
        }
    ...
    protected void updateServerConfig(ServerConfig config) {
        ...
        if (this.localServerID == config.getServerID())
            return; // this config musn't be updated

        int ind = this.remoteServerTable.indexOf("Server-" + config.getServerID() + "-Version");

        if (ind >= 0) {
            ind = this.remoteServerTable.indexOf("=", ind);
            ...
            if (ind >= 0) {
                ...
                if (end >= 0) {
                    ...
                    String version = this.remoteServerTable.substring(ind, end).trim();
                    ...
                    if (!config.getConfigVersion().equals(version)) {
                        ...
                        String fileURL = this.remoteServerConfigHomeURL + ServerConfigManager.SERVERS_PREFIX + config.getServerID() + ServerConfigManager.SERVERS_SUFFIX;
                        ...
                        String newAdr = FileTools.getTextFileFromURL(fileURL + ServerConfigManager.SERVERS_ADDRESS_SUFFIX);
    ...
    private int getMaxValue() {
        ...
        while ((ind = this.remoteServerTable.indexOf("Server-", ind)) > 0) {
            nb++;
    ...
    public void getLatestConfigFiles(Component parent) {
        ...
        while ((ind = this.remoteServerTable.indexOf("Server-", ind)) > 0) {
            ...
            int end = this.remoteServerTable.indexOf("-", ind);
            if (end < 0)
                return; // premature end of table

            // We retrieve the serverID
            int serverID = -1;
            try {
                serverID = Integer.parseInt(this.remoteServerTable.substring(ind, end));
            } catch (Exception e) {
            }
            ...
            if (serverID <= 0)
                continue; // negative or null IDs are skipped

            int versionBeg = this.remoteServerTable.indexOf("=", ind);
            int versionEnd = this.remoteServerTable.indexOf("\n", ind);

            if (versionBeg < 0 || versionEnd < 0) {
                break; // end of file
            }

            if (this.remoteServerTable.substring(versionBeg + 1, versionEnd).trim().length() == 0)
            ...
            if (config != null) {
                ...
                // creation
                if (parent != null && (pMonitor.isCanceled() || this.remoteServerTable.indexOf("WOT", ind) < 0)) {
                    pMonitor.close();
                    return;
                }
                ...
                String fileURL = this.remoteServerConfigHomeURL + ServerConfigManager.SERVERS_PREFIX + serverID + ServerConfigManager.SERVERS_SUFFIX;
                String newConfig = FileTools.getTextFileFromURL(fileURL);
    ...
    protected void addConfig(ServerConfig newConfig) {
        if (this.configs == null) {
            this.configs = new ServerConfig[1];
            this.configs[0] = newConfig;
        } else {
            ServerConfig myConfigs[] = new ServerConfig[this.configs.length + 1];
            System.arraycopy(this.configs, 0, myConfigs, 0, this.configs.length);
            myConfigs[this.configs.length] = newConfig;
    ...
    public ServerConfig getServerConfig(int serverID) {
        ...
        for (int i = 0; i < this.configs.length; i++)
            if (this.configs[i].getServerID() == serverID) {
                updateServerConfig(this.configs[i]); // trying to update
    ...
    public int getNextServerID(int previousServerID) {
        ...
        // Search the minimum
        for (int i = 0; i < this.configs.length; i++) {
            int id = this.configs[i].getServerID();
    ...
    public String reportDeadServer(int serverID) {
        ...
        if (currentConfig.getLastUpdateTime() + this.UPDATE_PERIOD > System.currentTimeMillis())
            return null; // we recently checked the address

        String fileURL = this.remoteServerConfigHomeURL + ServerConfigManager.SERVERS_PREFIX + currentConfig.getServerID() + ServerConfigManager.SERVERS_SUFFIX;
    ...
    public ServerConfig loadServerConfig(int serverID) {
        ...
        String serverFile = this.rManager.getExternalServerConfigsDir() + ServerConfigManager.SERVERS_PREFIX + serverID + ServerConfigManager.SERVERS_SUFFIX;

        ServerConfig config = (ServerConfig) this.rManager.loadObject(serverFile);
        String serverAdr = this.rManager.loadText(serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX);
        serverAdr = checkAddressFormat(serverAdr);

        if (config == null || serverAdr.length() == 0) {
            Debug.signal(Debug.ERROR, this, "Failed to load server config: " + "no valid server config" + serverID + " files found !");
            return null;
        }
    ...
    public boolean saveServerConfig(ServerConfig serverConfig) {
        ...
        String serverFile = this.rManager.getExternalServerConfigsDir() + ServerConfigManager.SERVERS_PREFIX + serverConfig.getServerID() + ServerConfigManager.SERVERS_SUFFIX;

        if (!this.rManager.saveObject(serverConfig, serverFile) || (serverConfig.getServerName() != null && !this.rManager.saveText(serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX, serverConfig.getServerName()))) {
            Debug.signal(Debug.ERROR, this, "Failed to save server config: " + "couldn't save " + serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX + " file !");
            return false;
        }
    ...
    public boolean updateServerConfig(String newConfigText, String newAdrText, ServerConfig oldServerConfig) {
        ...
        String serverFile = this.rManager.getExternalServerConfigsDir() + ServerConfigManager.SERVERS_PREFIX + oldServerConfig.getServerID() + ServerConfigManager.SERVERS_SUFFIX;

        if (newConfigText != null && !this.rManager.saveText(serverFile, newConfigText)) {
            Debug.signal(Debug.ERROR, this, "Failed to update server config: " + "failed to save " + serverFile + " file !");
            return false;
        }

        if (!this.rManager.saveText(serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX, newAdrText)) {
            Debug.signal(Debug.ERROR, this, "Failed to update server config: " + "failed to save " + serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX + " file !");
            return false;
        }

        // We load the newly saved config...
        if (newConfigText != null) {
            ServerConfig newConfig = (ServerConfig) this.rManager.loadObject(serverFile);

            if (newConfig.getServerID() != oldServerConfig.getServerID()) {
                Debug.signal(Debug.ERROR, this, "Failed to update server config: " + "new Config was not saved: it hasn't the expected Server ID !");
    ...
    public ServerConfig createServerConfig(String newConfigText, String newAdrText, int serverID) {
        ...
        File serversHomeDir = new File(this.rManager.getExternalServerConfigsDir());
        ...
        if (!this.rManager.saveText(serverFile, newConfigText))
    ...
    public ServerConfig[] loadServerConfigs() {
        ...
        String serversHome = this.rManager.getExternalServerConfigsDir();
        ...
        for (int i = 0; i < configFileList.length; i++)
            if (configFileList[i].isFile() && configFileList[i].getName().endsWith(ServerConfigManager.SERVERS_SUFFIX))
                nbFiles++;
        ...
        for (int i = 0; i < configFileList.length; i++)
            if (configFileList[i].isFile() && configFileList[i].getName().endsWith(ServerConfigManager.SERVERS_SUFFIX)) {
                ...
                configList[index] = (ServerConfig) this.rManager.loadObject(serverFile);
                ...
                String serverName = this.rManager.loadText(serverFile + ServerConfigManager.SERVERS_ADDRESS_SUFFIX);
