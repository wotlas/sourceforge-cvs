...
public class LieManager implements BackupReady {
    ...
    public void writeExternal(java.io.ObjectOutput objectOutput) throws java.io.IOException {
        ...
        for (int i = 0; i < fakeNamesLength; i++) {
            objectOutput.writeUTF(this.fakeNames[i]);
        }
        // Transfering currentFakeNameIndex
        objectOutput.writeShort(this.currentFakeNameIndex);
        // Transfering memories
        if (this.memories == null) {
            objectOutput.writeInt(0);
        } else {
            objectOutput.writeInt(this.memories.getSize());
            LieMemory memory;
            synchronized (this.memories) {
                this.memories.resetIterator();
                while (this.memories.hasNext()) {
    ...
    public void readExternal(java.io.ObjectInput objectInput) throws java.io.IOException, java.lang.ClassNotFoundException {
        ...
        if (IdTmp == ExternalizeGetVersion()) {
            ...
            for (int i = 0; i < fakeNamesLength; i++) {
                this.fakeNames[i] = objectInput.readUTF();
            }
            this.currentFakeNameIndex = objectInput.readShort();
