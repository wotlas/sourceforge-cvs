...
public class PropertiesConfigFile extends Properties {
    ...
    protected Object setProperty(String key, String value, boolean save) {
        ...
        String oldConfig = FileTools.loadTextFromFile(this.configFilePath);

        if (oldConfig == null) {
            Debug.signal(Debug.ERROR, this, "Failed to load " + this.configFilePath + " !");
            return obj;
        }

        oldConfig = FileTools.updateProperty(key, value, oldConfig);

        if (!FileTools.saveTextToFile(this.configFilePath, oldConfig))
            Debug.signal(Debug.ERROR, this, "Failed to save " + this.configFilePath + " !");
